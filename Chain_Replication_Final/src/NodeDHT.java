...
public class NodeDHT implements Runnable //extends UnicastRemoteObject implements NodeDHTInterface
{
    ...
    public static String considerInput(String received) throws Exception {
		...
		IDcounter++;
            	...
            	outResponse = getWord(tokens[1], tokens[2], tokens[3], tokens[4]);
        }
    	else if (tokens[0].equals("getWordLast")){
    		outResponse = getWordLast(tokens[1], Integer.parseInt(tokens[2]), tokens[3], tokens[4], tokens[5]);
    	}
	...
	public static String updateValues(int lower, int upper, String all){
		//System.out.println("Inside update Values , my id is " + Integer.toString(me.getID())); 
		String response = "";
	        Iterator<Word> iterator = wordList.iterator();
		if(all.equals("yes")){
	    		while (iterator.hasNext()) {
	...
	public static String getWord(String word, String origIP, String origPort, String count) throws Exception{
		String response = "";
		String request = "";
		String[] chunks;
		boolean found = false;
		...
		if(found){
			...
			request = "reply/" + count +": "+ word +" was not found!";
		}
    	...
    	public static String lookupKey(int key, String word, String origIP, String origPort, String count) throws Exception {
        	...
        	String request = "getWordLast/" +  word + "/" + Integer.toString(replList.size()) + "/" + origIP +"/" + origPort +"/" +count;
        	String response = "";
        	response = makeConnection(destNode.getIP(),destNode.getPort(),request);
        	return response;
    	}

	public static String getWordLast(String word, int m, String origIP, String origPort, String count) throws Exception{
		//System.out.println("I am node "+ Integer.toString(me.getID()));
		String request;
		String response="";
		if(m==0){
			//System.out.println("i have last replica  for word "+word);
			request = "getWord/" + word + "/" + origIP + "/" + origPort + "/" + count;
			response = makeConnection(me.getIP(), me.getPort(), request);
		}
		else{
			request = "getWordLast/" + word + "/" + Integer.toString(m-1) + "/" + origIP +"/" + origPort + "/" + count;
			response = makeConnection(finger[1].getSuccessor().getIP(), finger[1].getSuccessor().getPort(), request);
		}
		return response;
	}
		...
		for(Word w : wordList){
			if(w.getWord().equals(word)){
				...
				String request = "updateReplicas/" + key + "/" + word + "/" + meaning + "/" + Integer.toString(k-1) + "/" + Integer.toString(me.getID()) + "/" + origIP + "/" + origPort +"/" + count;
				makeConnection(finger[1].getSuccessor().getIP(), finger[1].getSuccessor().getPort(), request);
				break;
			}
		}
		if(notExists){
			...
			String request = "addReplicas/" + key + "/" + word + "/" + meaning + "/" + Integer.toString(k-1) + "/" + Integer.toString(me.getID()) + "/" + origIP + "/" + origPort + "/" + count;
			makeConnection(finger[1].getSuccessor().getIP(), finger[1].getSuccessor().getPort(), request);		
    		}
    	}
		...
		if((me.getID()==coordID) || k == 0 ){
			request = "reply/" + count + " : " +  word + " was added!";
	...
	public static void updateReplicas(int key, String word, String meaning, int k, int coordID, String origIP, String origPort, String count) throws Exception{
		String request="";
		if(me.getID()==coordID || k==0){
			request = "reply/" + count +" : " + word +" was updated!";
			Runnable runnable2 = new ThreadWorker(origIP, origPort, request);
        	   	Thread t = new Thread(runnable2);
            		t.start();
			return;
		...
		if(found){
			//System.out.println("Key-word deleted!!");
        	}else{
	...
	public static void departUpdateList(int nodeID, String nodeIP, String nodePort,int position, int m) throws Exception{
		...
		if(m==0){
			...
			//System.out.println("I send request : "+ request);
